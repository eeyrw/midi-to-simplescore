...
| 多数事件单字节编码 | 6-bit note 直接覆盖 0~61（5 个八度） |
...
直连 note 范围 0~61 覆盖 5 个八度。移调系统将曲目中心拉到 note 31 附近，可保证绝大多数音符落在直连范围内。

音符 62~127 使用扩展编码，需要 2 个 event 字节。编码选择由生成器在转换时决定，解析器无需关心。
...
1. **直连范围 0~61**: 生成器应利用移调将曲目中心拉到 note 31 附近，最大化直连命中率。
